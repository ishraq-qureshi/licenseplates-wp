...
foreach ( $items as $item_id => $item ) :
	...
	if ( is_object( $product ) ) {
		...
		if ( $product->get_type() === 'lptvplate' ) {
			...
			if ( $productsMetaImage ) {
				$image_url = site_url('/wp-content/plugins/lptv-plates/images/' . $productsMetaImage);
			} else {
				$image_url = site_url('/wp-content/plugins/lptv-plates/images/' . strtolower($modelId) . '.gif');
			}

			$image = '<img src="' . esc_url($image_url) . '" width="' . $image_size[0] . '" height="' . $image_size[1] . '" />';
	...
	<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
		<td class="td font-family text-align-left" style="vertical-align: middle; word-wrap:break-word;">
			...
			// display custom product instructions if available (admin only) - works for both layouts
			if ( $sent_to_admin && $product && $product->get_meta( '_plate_products_instructions' ) ) {
				...
				if ( ! empty( $instructions ) ) {
					echo '<br/><br/>Manufacturing Instructions:<br/>' . wp_kses_post( $instructions );
				}
			}
			?>
		</td>
