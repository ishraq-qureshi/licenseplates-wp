...
function zen_display_all_banner1($action, $identifier,$titleflag=true) {
    global $db, $request_type,$banner_array;
	$banner_string='';
    if ($action == 'dynamic') {
        $banner = $db->Execute("select banners_id, banners_title, banners_image, banners_html_text, banners_open_new_windows, banners_url
                               from " . TABLE_BANNERS . "
                               where status = 1 and banners_group='".$identifier."' order by banners_id desc");
		while (!$banner->EOF){
		$bannertitle='';
		if($titleflag) $bannertitle=$banner->fields['banners_title'];
		$temptitle='';
      if ($banner->fields['banners_url'] == '') {
        $temptitle=substr($banner->fields['banners_image'],0,strpos($banner->fields['banners_image'], '.'));
        $temptitle=str_replace("-"," ",$temptitle);
        $temptitle='Custom California license plates - personalized plates for your collector car, vintage automobile, or hot rod';
        //$banner_string .= zen_image(DIR_WS_IMAGES . $banner->fields['banners_image'], $temptitle);
        $banner_string .= '<li data-transition="fade" data-slotamount="3" data-masterspeed="300" data-delay="9000"><img src="'.DIR_WS_IMAGES . $banner->fields['banners_image'].'" alt="'.$temptitle.'" class="defaultimg" /></li>';
      } else {
        if ($banner->fields['banners_open_new_windows'] == '1') {
		  $banner_string .= '<li data-transition="fade" data-slotamount="3" data-masterspeed="300" data-delay="9000"><a href="' . zen_href_link(FILENAME_REDIRECT, 'action=banner&goto=' . $banner->fields['banners_id']) . '" target="_blank">
		  <img src="'.DIR_WS_IMAGES . $banner->fields['banners_image'].'" alt="'.$bannertitle.'" class="defaultimg" />
		  '. '</a></li>';
        } else {
          $banner_string .= '<li data-transition="fade" data-slotamount="3" data-masterspeed="300" data-delay="9000"><img src="'.DIR_WS_IMAGES . $banner->fields['banners_image'].'" alt="'.$bannertitle.'" class="defaultimg" /></li>';
        }
      }
	  	$banner->MoveNext();
		}
    ...
    zen_update_banner_display_count($banner->fields['banners_id']);
    $banner_array.=$banner->fields['banners_id'];
    return $banner_string;
  }