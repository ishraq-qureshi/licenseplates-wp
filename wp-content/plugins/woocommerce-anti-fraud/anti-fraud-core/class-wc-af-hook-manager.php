...
if ( ! class_exists( 'WC_AF_Hook_Manager' ) ) {
	...
	class WC_AF_Hook_Manager {
		...
		private function __construct() {
			...
			add_action( 'add_meta_boxes', array( $this, 'add_maxmind_meta_boxes' ) );
			add_action( 'add_meta_boxes', array( $this, 'add_ai_fraud_prevention_meta_boxes' ) );
		 ...
		 * @since  5.8.0
		 */
		public function add_maxmind_meta_boxes() {
			new WC_AF_Maxmind_Meta_Box();
		}
		 ...
		 * @since  5.8.0
		 */
		public function add_ai_fraud_prevention_meta_boxes() {
			new WC_AF_Ai_Fraud_Prevention_Meta_Box();
		}
		...
		public function change_order_status( $id, $old_status, $new_status ) {
			...
			if ( 'completed' == $new_status || 'processing' == $new_status || 'on-hold' == $new_status ) {
				...
				// When an admin manually moves an order OUT of "failed", check whether
				// the customer is now covered by a whitelist entry.  If they are, force
				// an immediate re-evaluation so the whitelist is applied right away –
				// clearing the stale high-risk score and preventing any automatic revert.
				if ( 'failed' === $old_status ) {
					$order = wc_get_order( $id );
					if ( $order && $score_helper->is_order_currently_whitelisted( $order ) ) {
						$score_helper->schedule_fraud_check( $id, true, '', true );
						return;
					}
				}

				// Default path: schedule check (skipped when a valid score already exists).
		...
		*/
		...
		public function do_this_hourly() {
			...
			/*
			 * PERFORMANCE FIX: This cron fires every hour to catch any orders that slipped
			 * through the normal fraud-check pipeline.  Previously it ran wc_get_orders with
			 * limit=-1 which could scan thousands of rows in a large store, and had no
			 * concurrency guard — so multiple WP-cron processes could run simultaneously,
			 * each issuing the same expensive query.
			 *
			 * Changes:
			 *  1. Mutex lock: only one process runs the heavy query at a time.
			 *  2. Per-run batch cap: processes at most 50 orders per hourly run so the total
			 *     lock hold-time stays short.  Remaining orders are handled on the next tick.
			 */

			// Mutex: bail if another instance of this cron is already running.
			$lock_key = 'wc_af_hourly_lock';
			if ( get_transient( $lock_key ) ) {
				Af_Logger::debug( 'do_this_hourly: skipped – another instance is already running.' );
				return;
			}
			// Hold the lock for up to 55 seconds (just under the 1-hour interval).
			set_transient( $lock_key, true, 55 );
			...
			$date_to   = gmdate( 'Y-m-d', strtotime( '-' . $num_days . ' days' ) );

			/**
			 * Filters the maximum number of orders processed per hourly auto-check run.
			 *
			 * Increasing this value means more orders are checked per cron tick but each
			 * run takes longer.  Reducing it keeps each run short at the cost of needing
			 * more ticks to clear a large backlog.  Default: 50.
			 *
			 * @since 7.2.6
			 *
			 * @param int $batch_size Max orders per hourly run.
			 */
			$batch_size = (int) apply_filters( 'wc_af_hourly_batch_size', 50 );
		...
		public function preapproved_api_order( $details ) {
			...
			if ( ! empty( $payer_email ) && ! empty( $data ) ) {
				...
				$order_id = isset( $tmp_data->order_id ) ? absint( $tmp_data->order_id ) : 0;
				if ( ! $order_id ) {
					Af_Logger::debug( 'preapproved_api_order: missing order ID in PayPal IPN payload, skipping.' );
					return;
				}

				$order = wc_get_order( $order_id );
				if ( ! $order || ! is_a( $order, 'WC_Order' ) ) {
					Af_Logger::debug( 'preapproved_api_order: order not found for ID ' . $order_id );
					return;
				}

				opmc_hpos_update_post_meta( $order_id, '_paypal_payer_email', $payer_email );
				$score_helper->paypal_email_verification( $order, 10 );
			}
		}
		...
		public function get_buyer_paypal_express_email( $details ) {
			...
			if ( ! empty( $payer_email ) && ! empty( $data ) ) {
				...
				$order_id = isset( $tmp_data->order_id ) ? absint( $tmp_data->order_id ) : 0;
				if ( ! $order_id ) {
					Af_Logger::debug( 'get_buyer_paypal_express_email: missing order ID in PayPal IPN payload, skipping.' );
					return;
				}

				$order = wc_get_order( $order_id );
				if ( ! $order || ! is_a( $order, 'WC_Order' ) ) {
					Af_Logger::debug( 'get_buyer_paypal_express_email: order not found for ID ' . $order_id );
					return;
				}

				opmc_hpos_update_post_meta( $order_id, '_paypal_express_payer_email', $payer_email );
				$score_helper->paypal_email_verification( $order, 10 );
