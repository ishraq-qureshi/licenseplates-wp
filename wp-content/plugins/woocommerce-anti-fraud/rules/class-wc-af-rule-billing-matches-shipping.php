...
if ( ! class_exists( 'WC_AF_Rule_Billing_Matches_Shipping' ) ) {
	class WC_AF_Rule_Billing_Matches_Shipping extends WC_AF_Rule {
		...
		public function __construct() {
			...
			parent::__construct( 'billing_matches_shipping', __('Billing address does not match shipping address', 'woocommerce-anti-fraud'), $this->rule_weight );
		}
		...
		// Check if the billing address does not match shipping address
			if ( $this->has_shipping_address( $order ) ) {
				...
				$add_diff = levenshtein( $billing_address, $shipping_address );
				// Check if the billing address does not match shipping address.
				if ( $add_diff > 7 ) {
					$risk = true;
					...
					'billing_address' => strip_tags( $billing_address ),
					'shipping_address' => strip_tags( $shipping_address ),
					'difference_score' => $add_diff . ' characters different',
					) );
				
					$this->add_explanation_item( 'reason', sprintf(
						'Billing and shipping addresses differ significantly (difference score: %d, threshold: 7)',
						$add_diff
					) );
