...
class WC_AF_Rule_MinFraud_Factors extends WC_AF_Rule {
	...
	public function __construct() {
		...
		parent::__construct( 'minfraud', __('Score returned by Minfraud Factors exceeds the allowed value.', 'woocommerce-anti-fraud'), $this->rule_weight );
	}
	...
	public function call_maxmind_api_on_order_place( $order ) {
		...
		$result = WC_AF_MaxMind_API_Client::check_order( $order, 'factors' );
		
		if ( isset( $result['error'] ) ) {
			Af_Logger::debug( 'minfraud factors error: ' . $result['error'] );
		...
		Af_Logger::debug( 'minfraud factors score ' . $result['risk_score'] );
