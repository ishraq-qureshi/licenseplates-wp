...
class WC_AF_Rule_Free_Email extends WC_AF_Rule {
	...
	public function __construct() {
		...
		parent::__construct( 'free_email', __('Email is a known free email address.', 'woocommerce-anti-fraud'), $this->rule_weight );
	}
	...
	// Do the regex
	$regex_result = preg_match( '`@([a-zA-z0-9\-\_]+)(?:\.[a-zA-Z]{0,5}){0,2}$`', $billing_email, $email_domain );
		...
		if ( 1 === $regex_result ) {
			...
			if ( in_array( $email_domain[1], $free_email_domains ) ) {
				...
				$this->add_explanation_item( 'reason', sprintf(
					'Email domain "%s" is a known free email provider',
					$email_domain[1]
				) );
