...
class WC_AF_Rule_Country extends WC_AF_Rule {
	...
	public function __construct() {
		...
		parent::__construct( 'country', __('Ordered from a risk country', 'woocommerce-anti-fraud'), $this->rule_weight );
	}
	...
	public function is_risk( WC_Order $order ) {
		...
		if ( true === $risk ) {
			...
			if ( $store_country == $billing_country && $store_country == $shipping_country ) {
				...
				$flagged_countries = array();
				if ( in_array( $billing_country, $risk_countries ) ) {
					$flagged_countries[] = 'Billing: ' . $billing_country_name;
				}
				if ( in_array( $shipping_country, $risk_countries ) ) {
					$flagged_countries[] = 'Shipping: ' . $shipping_country_name;
				}
			
				$this->add_explanation_item( 'reason', 'Order from configured high-risk country: ' . implode( ', ', $flagged_countries ) );
