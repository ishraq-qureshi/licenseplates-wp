...
class WC_AF_Rule_MinFraud_Insights extends WC_AF_Rule {
	...
	public function __construct() {
		$this->minimum_minfraud_score   = get_option( 'wc_settings_anti_fraud_minfraud_insights_risk_score' );
		$this->is_enabled  = get_option( 'wc_af_maxmind_insights' );
		$this->rule_weight = get_option( 'wc_settings_anti_fraud_minfraud_insights_order_weight' );
		parent::__construct( 'minfraud', __('Score returned by Minfraud Insights exceeds the allowed value.', 'woocommerce-anti-fraud'), $this->rule_weight );
	}
	...
	public function is_risk( WC_Order $order ) {
		Af_Logger::debug( 'Checking minfraud insights rule' );
		global $wpdb;
	...
	public function call_maxmind_api_on_order_place( $order ) {
		...
		$result = WC_AF_MaxMind_API_Client::check_order( $order, 'insights' );
		
		if ( isset( $result['error'] ) ) {
			Af_Logger::debug( 'minfraud insights error: ' . $result['error'] );
		...
		Af_Logger::debug( 'minfraud insights score ' . $result['risk_score'] );
