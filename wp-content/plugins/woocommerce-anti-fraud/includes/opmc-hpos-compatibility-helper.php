...
if ( ! function_exists( 'opmc_hpos_update_post_meta' ) ) {
	 ...
	 * @return bool True when meta was updated, false when skipped or order not found.
	 */
	function opmc_hpos_update_post_meta( $post_id, $meta_key, $meta_value ) {
		$post_id = absint( $post_id );
		if ( ! $post_id ) {
			if ( class_exists( 'Af_Logger' ) ) {
				Af_Logger::debug( 'opmc_hpos_update_post_meta: skipped invalid order ID.' );
			}
			return false;
		}
		...
		if ( class_exists( 'Automattic\WooCommerce\Utilities\OrderUtil' ) && OrderUtil::custom_orders_table_usage_is_enabled() ) {
			...
			if ( ! $order || ! is_a( $order, 'WC_Order' ) ) {
				if ( class_exists( 'Af_Logger' ) ) {
					Af_Logger::debug( 'opmc_hpos_update_post_meta: order not found for ID ' . $post_id );
				}
				return false;
			}
...
if ( ! function_exists( 'opmc_hpos_delete_post_meta' ) ) {
	 ...
	 * @param string $meta_key   Meta key.
	 * @param mixed  $meta_value Meta value.
	 * @return bool True when meta was deleted, false when skipped or order not found.
	 */
	function opmc_hpos_delete_post_meta( $post_id, $meta_key, $meta_value ) {
		$post_id = absint( $post_id );
		if ( ! $post_id ) {
			if ( class_exists( 'Af_Logger' ) ) {
				Af_Logger::debug( 'opmc_hpos_delete_post_meta: skipped invalid order ID.' );
			}
			return false;
		}
		...
		if ( class_exists( 'Automattic\WooCommerce\Utilities\OrderUtil' ) && OrderUtil::custom_orders_table_usage_is_enabled() ) {
			...
			if ( ! $order || ! is_a( $order, 'WC_Order' ) ) {
				if ( class_exists( 'Af_Logger' ) ) {
					Af_Logger::debug( 'opmc_hpos_delete_post_meta: order not found for ID ' . $post_id );
				}
				return false;
			}
