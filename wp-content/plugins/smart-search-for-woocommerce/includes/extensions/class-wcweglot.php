...
class WcWeglot extends Abstract_Extension {
	...
	public function searchaniseGetTranslate( $content, $currently_language ) {
		...
		if ( $this->is_active() && $this->isAvailableIntegration() && function_exists( 'weglot_get_service' ) ) {
			...
			$normalize_current_language = $this->normalizeLanguage( $currently_language );

			if ( $normalize_current_language != $currently_language ) {
				try {
	...
	private function normalizeLanguage( $language ) {
		return strtolower( explode( '_', $language )[0] );
	}
}
