...
use SkyVerge\WooCommerce\PluginFramework\v6_1_2\Helpers\OrderHelper;
use SkyVerge\WooCommerce\PluginFramework\v6_1_2 as Framework;
use SkyVerge\WooCommerce\Authorize_Net\DataObjects\ScriptRegistration;
...
class WC_Gateway_Authorize_Net_CIM extends Framework\SV_WC_Payment_Gateway_Direct {
	...
	public function admin_options() {
		...
		( function( $ ) {
			...
			$( '.form-type-select' ).change( function() {
				...
				if ( '<?php echo esc_js( self::FEATURE_PAYMENT_FORM_HOSTED ); ?>' === value ) {
					...
					var description = '<?php esc_attr_e( 'Display an embedded payment form on the pay page for PCI-DSS SAQ A compliance.', 'woocommerce-gateway-authorize-net-cim' ); ?>';
					...
					if ( '<?php echo esc_js( self::FEATURE_PAYMENT_FORM_LIGHTBOX ); ?>' === value ) {
						var description = '<?php esc_attr_e( 'Display the payment form as a lightbox at checkout for PCI-DSS SAQ A compliance.', 'woocommerce-gateway-authorize-net-cim' ); ?>';
					} else {
						var description = '<?php esc_attr_e( 'Display the payment form inline at checkout for PCI-DSS SAQ A-EP compliance.', 'woocommerce-gateway-authorize-net-cim' ); ?>';
					}
		...
		Framework\Helpers\ScriptHelper::addInlineScript('woocommerce-'.$this->get_id().'-custom', ob_get_clean() );
	}
	...
	protected function enqueue_gateway_assets() {
		...
		$scriptRegistration = $this->getAcceptJsScriptRegistration();

		if ( $scriptRegistration && $this->should_enqueue_accept_js() ) {
			parent::enqueue_gateway_assets();
			// null $ver prevents warnings from AcceptJS caused by WP versioning
			wp_enqueue_script( $scriptRegistration->handle, $scriptRegistration->url, [], null, true );
		}
	}

	/**
	 * Gets the Accept.js / AcceptUI.js script registration for the current payment form type.
	 *
	 * @since 3.10.13
	 */
	public function getAcceptJsScriptRegistration() : ?ScriptRegistration
	{
		if ($this->is_lightbox_payment_form_enabled()) {
			$url    = $this->getLightboxJsUrl();
		...
		return new ScriptRegistration($url, $handle);
	}

	/**
	 * Gets the URL for the Lightbox AcceptUI.js script for the current environment.
	 *
	 * @since 3.10.13
	 */
	public function getLightboxJsUrl() : string
	{
		return $this->is_production_environment() ? 'https://js.authorize.net/v3/AcceptUI.js' : 'https://jstest.authorize.net/v3/AcceptUI.js';
	}


	/**
	 * Determines if Accept JS should be enqueued.
	 *
	 * This is mainly necessary because WooCommerce loads the checkout block code on the order received page as well.
	 * However our checkout block code requires Accept.js and if not enqueued it will trigger an error.
	 *
	 * @since 3.10.-dev.1
	 *
	 * @return bool
	 */
	protected function should_enqueue_accept_js() : bool {
		$isCheckoutBlockInUse = Framework\Blocks\Blocks_Handler::is_checkout_block_in_use();

		// If the checkout block is in use and the payment form type is lightbox, don't enqueue Accept.js
		// The checkout block will handle it in wp-plugin-frontend
		// This is because we cannot load Accept JS until _after_ the input field with the handler is rendered in the DOM.
		if ($isCheckoutBlockInUse && $this->is_lightbox_payment_form_enabled() && ! is_add_payment_method_page()) {
			return false;
		}

		if ( $isCheckoutBlockInUse ) {
			$should_load_on_order_received_page = true;
		} else {
			$should_load_on_order_received_page = ! is_order_received_page();
		}

		return $should_load_on_order_received_page
			&& ( is_checkout() || is_checkout_pay_page() || is_add_payment_method_page() );
	}
	...
	public function do_transaction( $order ) {
		...
		/** @var WC_Authorize_Net_CIM_Payment_Profile|null $token */
		$token = $this->get_payment_tokens_handler()->get_token( $order->get_user_id(), $payment->token );

		if ( ! $token ) {
			throw new Framework\SV_WC_Plugin_Exception( __( 'Invalid payment token', 'woocommerce-gateway-authorize-net-cim' ) );
		}
		...
		if ( empty( $payment->shipping_address_id ) ) {
			...
			$payment->shipping_address_id = is_numeric( $response ) ? $response : $response->get_shipping_address_id();

			// persist profile ID to user meta
			$shipping_address->update_id( $payment->shipping_address_id );
	...
	public function get_order( $order_id ) {
		...
		$description = OrderHelper::get_property( $order, 'description', null, '' );

		// backwards compat for transaction/PO number filters introduced in v1.x
		// @deprecated in 2.0.0
		$description = apply_filters( 'wc_authorize_net_cim_transaction_description', $description, $order_id, $this );

		// remove any weirdness in the description
		$description = Framework\SV_WC_Helper::str_to_sane_utf8( $description );

		OrderHelper::set_property( $order, 'description', $description );

		// @deprecated in 2.0.0
		$payment = OrderHelper::get_payment( $order );
		$payment->po_number = apply_filters( 'wc_authorize_net_cim_transaction_po_number', '', $order_id, $this );

		// add shipping address ID for profile transactions (using existing payment method or adding a new one)
		if ( $order->get_user_id() && ( ! empty( $payment->token ) || $this->get_payment_tokens_handler()->should_tokenize() ) ) {

			$shipping_address = $this->get_shipping_address( $order->get_user_id() );

			$payment->shipping_address_id = $shipping_address->get_id();
		}

		if ( $last_four = Framework\SV_WC_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-last-four' ) ) {
			$payment->account_number = $payment->last_four = substr( $last_four, -4 );
		}

		// nonce data
		if ( empty( $payment->opaque_value ) ) {
			$payment->opaque_descriptor = Framework\SV_WC_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-payment-descriptor' );
			$payment->opaque_value      = Framework\SV_WC_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-payment-nonce' );
		}
	...
	public function get_order_meta( $order, $key ) {
		...
		if ( 'payment_token' === $key && ! parent::get_order_meta( $order_id, $key ) && $this->get_plugin()->is_subscriptions_active() && wcs_order_contains_renewal( $order ) ) {
			...
			if ( $parent_order instanceof \WC_Order && $parent_token = $parent_order->get_meta( '_wc_authorize_net_cim_payment_profile_id' ) ) {
		...
		if ( 'payment_token' === $key && $order->meta_exists($this->get_order_meta_prefix() . $key ) && $order->meta_exists( '_wc_authorize_net_cim_payment_profile_id' ) ) {

			$legacy_payment_token = $order->get_meta( '_wc_authorize_net_cim_payment_profile_id' );
			$payment_token        = $order->get_meta( $this->get_order_meta_prefix() . 'payment_token' );
			$customer_id          = $order->get_meta( '_wc_authorize_net_cim_customer_profile_id' );
		...
		if ( 'payment_token' === $key && ! $order->meta_exists( $this->get_order_meta_prefix() . $key ) && $order->meta_exists( '_wc_authorize_net_cim_payment_profile_id' ) ) {
			...
			$token = $order->get_meta( '_wc_authorize_net_cim_payment_profile_id' );
		...
		if ( 'customer_id' === $key && ! $order->meta_exists( $this->get_order_meta_prefix() . $key ) && $order->meta_exists( '_wc_authorize_net_cim_customer_profile_id' ) ) {
		...
		if ( 'trans_id' === $key && ! $order->meta_exists( $this->get_order_meta_prefix() . $key ) && $order->get_transaction_id() ) {
			$this->update_order_meta( $order, 'trans_id', $order->get_transaction_id() );
		}

		return $order->get_meta( $this->get_order_meta_prefix() . $key );
	}
	 ...
	 *
	 * @since 2.0.0
	 * @param string|null $environment_id optional one of 'test' or 'production', defaults to current configured environment
	 ...
	 * @since 2.8.0
	 *
	 * @param string|null $environment_id one of 'test' or 'production', defaults to current configured environment
	 * @return string
	 */
