...
class WC_Authorize_Net_CIM_Payment_Profile extends Framework\SV_WC_Payment_Gateway_Payment_Token {
	...
	protected function parse_data_from_order( WC_Order $order ) {
		...
		$data = [
			...
			'customer_profile_id' => OrderHelper::get_customer_id( $order ),
		];

		if ( 'credit_card' === $data['type'] ) {

			$data['card_type'] = isset( $payment->card_type ) ? $payment->card_type : Framework\SV_WC_Payment_Gateway_Helper::card_type_from_account_number( $payment->account_number );
			$data['exp_month'] = $payment->exp_month;
			$data['exp_year']  = $payment->exp_year;

		} elseif ( 'echeck' === $data['type'] ) {

			$data['account_type'] = $payment->account_type;
		}
	...
	public function is_duplicate_of( WC_Order $order ) {
		...
		$entered_payment = [
			'last_four' => $payment->last_four,
			'type'      => 'credit_card' === $payment->type ? $payment->card_type : $payment->account_type,
			'postcode'  => $order->get_billing_postcode(),
		];
