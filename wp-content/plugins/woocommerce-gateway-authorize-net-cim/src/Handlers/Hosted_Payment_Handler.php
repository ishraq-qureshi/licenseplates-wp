...
class Hosted_Payment_Handler extends Framework\Payment_Gateway\Handlers\Abstract_Hosted_Payment_Handler {
	...
	public function payment_page( $order_id ) {
		...
		$payment->communicator_url = $this->get_iframe_communicator_url();
	...
	protected function render_payment_form( $token, \WC_Order $order ) {
		...
		Framework\Helpers\ScriptHelper::addInlineScript(
			'wc' . $this->get_gateway()->get_id_dasherized() . '-accept-hosted',
			sprintf( 'window.wc_authorize_net_accept_hosted_handler = new WC_Authorize_Net_Accept_Hosted_Handler( %s );', json_encode( $args ) )
		);

		echo '<form id="wc-' . esc_attr( $this->get_gateway()->get_id_dasherized() ) . '-send-token" action="' . esc_url( $this->get_hosted_payment_url() ) . '" method="post" target="wc-' . esc_attr( $this->get_gateway()->get_id_dasherized() ) . '-iframe" >';
		echo '<input type="hidden" name="token" value="' . esc_attr( $token ) . '" />';
		echo '</form>';

		echo '<iframe id="wc-' . esc_attr( $this->get_gateway()->get_id_dasherized() ) . '-iframe" name="wc-' . esc_attr( $this->get_gateway()->get_id_dasherized() ) . '-iframe" width="100%" height="650px" frameborder="0" scrolling="no" hidden="true"></iframe>';
	}
