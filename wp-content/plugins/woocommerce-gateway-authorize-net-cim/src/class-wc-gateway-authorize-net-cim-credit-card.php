...
class WC_Gateway_Authorize_Net_CIM_Credit_Card extends WC_Gateway_Authorize_Net_CIM {
	...
	public function get_order( $order_id ) {
		...
		if ( empty( $payment->token ) && ! Framework\SV_WC_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-payment-token' ) ) {

			// expiry month/year
			if ( $expiry = Framework\SV_WC_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-expiry' ) ) {
				[ $payment->exp_month, $payment->exp_year ] = array_map( 'trim', explode( '/', $expiry ) );
			}

			$payment->card_type = Framework\SV_WC_Helper::get_posted_value( 'wc-' . $this->get_id_dasherized() . '-card-type' );
		}
	...
	public function get_order_for_apple_pay( WC_Order $order, Framework\SV_WC_Payment_Gateway_Apple_Pay_Payment_Response $response ) {
		...
		// opaque data
		$payment = OrderHelper::get_payment( $order );
		$payment->opaque_value      = base64_encode( json_encode( $response->get_payment_data() ) );
		$payment->opaque_descriptor = 'COMMON.APPLE.INAPP.PAYMENT';
		OrderHelper::set_payment( $order, $payment );
	...
	protected function get_order_for_refund( $order_id, $amount, $reason ) {
		...
		try {
			...
			$response = $this->get_api()->get_transaction_details( $refund->trans_id, $order->get_id() );

			$refund->last_four = $response->get_last_four();

			// if this is a partial refund, find out if the transaction has settled before continuing
			if ( $refund->amount != $order->get_total() ) {
		...
		if ( empty( $refund->last_four ) ) {
			$refund->last_four = $this->get_order_meta( $order, 'account_four' );
		}

		if ( $expiry_date = $this->get_order_meta( $order, 'card_expiry_date' ) ) {
			$refund->expiry_date = date( 'm-Y', strtotime( '20' . $expiry_date ) );
		} else {
			$refund->expiry_date = 'XXXX';
		}

		// we must error out if all above attempts at retrieving the card last four fail
		if ( empty( $refund->last_four ) || empty( $refund->expiry_date ) ) {
