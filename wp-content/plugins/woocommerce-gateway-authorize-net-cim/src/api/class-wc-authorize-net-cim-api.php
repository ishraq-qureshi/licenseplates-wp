...
class WC_Authorize_Net_CIM_API extends Framework\SV_WC_API_Base implements Framework\SV_WC_Payment_Gateway_API {
		...
		try {
			...
			if ( 39 == $e->getCode() ) {
				...
				if ( preg_match( '/ID\s[0-9]*/', $e->getMessage(), $id ) ) {
					...
					OrderHelper::set_customer_id( $order, $id );
	...
	public function update_tokenized_payment_method( WC_Order $order ) {
		...
		$request = $this->get_new_request( 'payment-profile' );
		$request->get_payment_profile( OrderHelper::get_customer_id( $order ), $payment->token );
	...
	public function create_shipping_address( WC_Order $order ) {
		...
		try {
			...
			if ( 39 == $e->getCode() ) {
				...
				$request = $this->get_new_request( 'customer-profile' );
				$request->get_customer_profile( OrderHelper::get_customer_id( $order ) );
				$response = $this->perform_request( $request );
	...
	protected function get_transaction_request_type( WC_Order $order ) {
		...
		if ( ! empty( $refund ) ) {
			...
			return empty( $refund->customer_payment_profile_id ) ? 'non-profile-transaction' : 'profile-transaction';

		} else {
			$payment = OrderHelper::get_payment( $order );

			// all other transactions
			return empty( $payment->token ) ? 'non-profile-transaction' : 'profile-transaction';
		}
	}
