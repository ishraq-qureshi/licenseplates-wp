...
class URE_Posts_Edit_Access_User {
    ...
    public function is_restriction_applicable() {
        ...
        $is_admin = $this->lib->user_is_admin($current_user->ID);
    ...
    public function show_profile($user) {
        ...
        if ( stripos( $request_uri, 'network/user-edit.php') !== false) {  // exit, this code just for single site user profile only, not for network admin center
            return false;
        }
    ...
    private function get_post_types_from_roles() {
        ...
        foreach( $data as $item ) {
            ...
            if ( !empty( $item['data']['post_types'] ) ) {
                $post_types = ure_array_merge( $post_types, $item['data']['post_types'] );
            }
        }
    ...
    private function load_post_types() {
        ...
        if ( !empty( $roles_post_types ) ) {
            $post_types = ure_array_merge( $user_post_types, $roles_post_types );
        } else {
            $post_types = $user_post_types;
        }
    ...
    private function get_restricted_items_from_roles( $item_id ) {
        ...
        foreach( $items as $item ) {              
            ...
            $data1 = ure_array_merge( $data1, $data0 );
        }
    ...
    private function add_wc_orders_by_customer( $posts ) {
        ...
        $orders = $wpdb->get_col( $query );
        if ( !empty( $orders ) ) {
            $posts = ure_array_merge( $posts, $orders );
        }                        
     ...
     */
    private function get_post_children( $page_id, $all_pages, &$children ) {
        ...
        foreach($all_pages as $child_id=>$parent_id) {
            ...
            $children[] = $child_id;
            $this->get_post_children($child_id, $all_pages, $children);            
        }
    ...
    private function add_child_pages( $posts_list ) {
        ...
        $hierarchical_post_types = apply_filters('ure_auto_access_children_for_hierarchical_post_types', $hierarchical_post_types );
        if ( empty( $hierarchical_post_types ) || !is_array( $hierarchical_post_types ) ) {
            return $posts_list;
        }
        ...
        $all_hierarchical_posts = $wp_query->query( $args );
        
        $children = array();
        foreach( $posts_list as $post_id ) {
            $post = get_post( $post_id );
            if ( empty($post) || !in_array( $post->post_type, $hierarchical_post_types ) ) {
                continue;
            }

            $page_children = array();
            $this->get_post_children($post_id, $all_hierarchical_posts, $page_children);
            if (count($page_children)>0) {
                $children = ure_array_merge($children, $page_children);
            }                                    
        }

        if ( count( $children )>0 ) {
            $posts_list = ure_array_merge( $posts_list, $children );
        }
    ...
    private function add_product_variations( $posts_list ) {
        ...
        if ( count( $variations )>0 ) {
            $posts_list = ure_array_merge( $posts_list, $variations );
        }
    ...
    private function _get_posts_list( $post_type ) {
        ...
        $posts_list = array_values( array_unique( ure_array_merge( $posts_list1, $posts_list2, $posts_list3 ) ) );
    ...
    private function calc_is_restricted() {
        ...
        $own_data_only = $this->user_meta->get_own_data_only();  
        
        $restrictions = trim($post_types_str . $posts_list_str . $categories_list_str . $post_authors_list_str) . ( $own_data_only ? '1':'');
