...
class URE_Other_Roles {
    ...
    private function get_access_data_for_user_roles($user, $blocked) {
        ...
        foreach ($user->roles as $role) {
            ...
            if ( $access_model==$blocked['access_model'] && is_array( $data ) ) {
                $blocked['data'] = ure_array_merge( $blocked['data'], $data );
            }            
        }   // foreach(...)
    ...
    protected static function get_access_data_from_post() {
        ...
        $keys_to_skip = array(
            ...
            'user_role', 
            'ure_access_model');
        $access_model = $_POST['values']['ure_access_model'];
    ...
    public static function save_access_data_for_user($user_login) {
        $access_for_user = self::get_access_data_from_post();
        // TODO ...
    }
    ...
    public static function update_data() {
        ...
        if ( !current_user_can(URE_OTHER_ROLES_ACCESS::OTHER_ROLES_ACCESS_CAP ) ) {
        ...
        if ( $ure_object_type!=='role' && $ure_object_type!=='user') {
            $answer['message'] = esc_html__('URE: other roles access: Wrong object type. Data was not updated.', 'user-role-editor');
            return $answer;
        }
        ...
        if (empty( $ure_object_name ) ) {
            $answer['message'] = esc_html__('URE: other roles access: Empty object name. Data was not updated', 'user-role-editor');
            return $answer;
        }
        ...
        $answer = array(
            'result'=>'success', 
            'message'=>esc_html__('Other roles access data was updated successfully', 'user-role-editor')
            );
