...
<?php            
    ...
    public function act() {        
        ...
        $file_type = wp_check_filetype( $_FILES['roles_file']['name'], array('csv'=>'text/csv') );
        if ( strtoupper( $file_type['ext'] )!=='CSV' || strtoupper( $file_type['type'] )!=='TEXT/CSV') {
            $message = esc_html__('Wrong file type. CSV only is allowed', 'user-role-editor');
            $this->lib->show_message( $message, true );            
            return;
        } 
        
        $mime_type = URE_Utils::mime_type( $_FILES['roles_file']['tmp_name'] );
        if ( strtoupper( $mime_type )!=='TEXT/PLAIN' ) {
            $message = esc_html__('Wrong file type. CSV only is allowed', 'user-role-editor');
            $this->lib->show_message( $message, true );            
            return;
        }
