...
class URE_Posts_Edit_Access {
    ...
    public function set_hooks_general() {
        ...
        add_filter('list_terms_exclusions', array($this, 'exclude_terms'), 10 );        
    ...
    public function block_edit_post( $caps, $cap='', $user_id=0, $args=array() ) {
        ...
        remove_filter('map_meta_cap', array($this, 'block_edit_post'), 10 );  // do not allow endless recursion
    ...
    private function _restrict_posts_list( $query ) {
        ...
        if ( !empty( $query->query['post_type'] ) ) {
            $post_type = $query->query['post_type'];
        } else if ( !empty( $query->query_vars['post_type'] ) ) {
    ...
    public function exclude_terms( $exclusions ) {        
        ...
        if ( empty( $post_type ) ) {
            ...
            remove_filter('list_terms_exclusions', array($this, 'exclude_terms'), 10 );        
    ...
    private function assign_term_from_allowed($post_id, $terms_list_str, $registered_taxonomies) {
        ...
        $term_list_sec = URE_Base_Lib::esc_sql_in_list('int', $terms_list_str );
        $terms_list = explode(',', $term_list_sec );
    ...
    private function assign_term_having_not_allowed( $post_id, $not_allowed_terms_list_str, $registered_taxonomies ) {
        ...
        $query = $wpdb->prepare( "SELECT term_id FROM `". $wpdb->terms ."` WHERE term_id NOT IN (%s)", $term_list_str );
        ...
        foreach( $terms as $term ) {
            $query = $wpdb->prepare( "SELECT taxonomy FROM `". $wpdb->term_taxonomy ."` WHERE term_id=%d", $term->term_id );
            $taxonomy = $wpdb->get_var( $query );
            if ( empty( $taxonomy ) ) {
                continue;
            }
