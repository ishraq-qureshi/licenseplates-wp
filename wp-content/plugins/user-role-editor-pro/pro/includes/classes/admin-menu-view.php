...
<?php        
    ...
    private function update_profile_menu($allowed_caps) {
        ...
        $this->menu[70] = array( __('Profile', 'user-role-editor'), 'read', 'profile.php', 'profile.php');
...
?>
            <input type="checkbox" name="<?php echo esc_attr( $item_id );?>" id="<?php echo esc_attr( $item_id );?>" class="ure-cb-column" <?php echo $checked;?> />
<?php
    }
?>
        </td>
        ...
        <td></td>
        <td style="color:#cccccc;"><?php echo esc_html( $menu_item[1] );?></td>
        <td style="color:#cccccc; padding-left:10px;"><?php echo esc_html( $menu_item[3] );?></td>
    </tr>        
<?php
            ...
            foreach($this->submenu[$menu_item[2]] as $submenu_item) {
                ...
                $key_pos = false;
                if ( !empty( $submenu_item[0]) && ( strpos( $submenu_item[0], "<span class='update-count'")!==false ) ) {
...
?>
        ...
        <td></td>
        <td><?php echo wp_strip_all_tags( $menu_title ); ?></td>
        <td style="color:#cccccc;"><?php echo esc_html( $submenu_item[1] );?></td>
        <td style="color:#cccccc; padding-left:10px;"><?php echo esc_html( $submenu_item[3] );?></td>
    </tr>    
<?php    
...
?>
    ...
    <input type="hidden" name="ure_object_type" id="ure_object_type" value="<?php echo esc_attr( $ure_object_type );?>" />
...
<?php
    ...
    public function get_allowed_args_html() {
        ...
        $this->menu = get_option( URE_Admin_Menu::ADMIN_MENU_COPY_KEY, array() );
        $this->submenu = get_option( URE_Admin_Menu::ADMIN_SUBMENU_COPY_KEY, array() );
        $this->args_data = URE_Admin_Menu_URL_Allowed_Args::load_white_list();
        $lib = URE_Lib_Pro::get_instance();
        $all_caps = $lib->init_full_capabilities('role');
