...
private function check_roles_for_prohibited($roles, $post_id=0 ) {
    ...
    private function get_term_level_restrictions() {
        ...
        if ( !empty( $prohibited_posts ) ) {
            ...
            $this->prohibited_posts = ure_array_merge( $this->prohibited_posts, $prohibited_posts );
        }
    ...
    private function get_role_level_restrictions() {
        ...
        if ( count( $posts1 ) ) {
            $posts0 = ure_array_merge( $posts0, $posts1 );
        }
        ...
        if ( count($posts1) ) {
            ...
            $this->prohibited_posts = ure_array_merge( $this->prohibited_posts, $posts1 );
        }
    }
	...
	$user = wp_get_current_user();
        ...
        if ( !is_array($this->prohibited_posts) ) {
            ...
            set_transient( $transient_key2, self::$checked_posts, 30 );
        ...
        if (count($prohibited_posts)>0) {
            ...
            if (!empty($post_not_in)) {
                if (is_array($post_not_in)) {
                    $post_not_in = ure_array_merge( $post_not_in, $prohibited_posts );
                } else {
