...
<?php        
    ...
    private function get_users_to_block( $blocked ) {
        ...
        foreach( $blocked_roles as $role_id) {
            ...
            $ids_arr = ure_array_merge( $ids_arr, $ids_arr1 );
        }
    ...
    public function exclude_users_with_blocked_roles($user_query) {
        ...
        foreach ( $links_to_block as $link ) {
            $result = stripos( $request_uri, $link );
            if ($result !== false) {
                break;
            }
        }
    ...
    private function get_exclude_users_where( $blocked ) {
        ...
        if ( is_array( $blocked_users ) && count( $blocked_users ) > 0 ) {
             ...
             $result = $wpdb->prepare(' AND ( %i.user_id NOT IN ( %1s ) )', $wpdb->usermeta, $user_ids );
        } else {
            $result = ' AND 1=1';
        }
    ...
    private function get_full_count_users_sql( $site_id, $blocked ) {
        global $wpdb;
        
        $blog_prefix = $wpdb->get_blog_prefix( $site_id );
        $exclude_users_where = $this->get_exclude_users_where( $blocked );
        $query = $wpdb->prepare("SELECT meta_value FROM `". $wpdb->usermeta ."` INNER JOIN `". $wpdb->users ."` ". 
                                   "ON user_id=ID
                                    WHERE meta_key=CONCAT(%s, 'capabilities') %1s", 
                                $blog_prefix, $exclude_users_where );
        
        return $query;
    }
    // end of get_full_count_users_sql()
	...
	$result = array(); 
        ...
        $roles_id = array_keys( $roles );
        $editable_roles = $this->editable_roles( $roles_id );
        $avail_roles = array(
                    'none' => 0,
            );        
        $query = $this->get_full_count_users_sql( $site_id, $blocked );
        $users_of_blog = $wpdb->get_col( $query );
