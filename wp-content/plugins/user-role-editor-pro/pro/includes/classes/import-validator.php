    ...
    public static function validate_capabilities($role_id, $capabilities) {
        ...
        foreach($capabilities as $key=>$value) {            
            ...
            if ($key!==$sanitized_key) {    // illegial character found at the capability identifire
                ...
                $result->success = false;
                // translators: placeholders are replaced by: 1st: imported user role ID, 2nd: user capability ID, user capability value.
                $message = __('Import failure: Role "%1$s", wrong capability value for "%2$s" - only TRUE, FALSE, 1 and 0 are allowed, but "%3$s" found', 'user-role-editor');
