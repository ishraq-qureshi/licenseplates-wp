...
class URE_Meta_Boxes { 
    ...
    public function load_access_data_for_user( $user ) {
        ...
        foreach ( $user->roles as $role ) {
            if ( isset( $access_data[$role] ) ) {
                $blocked = ure_array_merge( $blocked, $access_data[$role] );
            }
        }
    ...
    protected static function get_access_data_from_post() {
    ...
    public static function save_access_data_for_role( $role_id ) {
        $access_for_role = URE_Meta_Boxes::get_access_data_from_post();
        $access_data = get_option( self::ACCESS_DATA_KEY );
    ...
    public static function save_access_data_for_user( $user_login ) {
...
?>
            <input type="checkbox" name="<?php echo esc_attr( $key );?>" id="<?php echo esc_attr( $key );?>" <?php echo $checked;?> />
<?php
    }
?>
        </td>
        <td class="ure-cell" style="width:45%;word-wrap:break-word;"><?php echo wp_strip_all_tags( $item->title );?></td>
        <td class="ure-cell" style="width:40%;word-wrap:break-word;"><?php echo esc_html( $item->id );?></td>    
        <td style="text-align: center;">
            <a href="#" onclick="ure_meta_boxes_access.remove_from_list('<?php echo esc_attr( $key ); ?>');" title="<?php esc_html_e('Delete from the list of available meta boxes', 'user-role-editor');?>">
