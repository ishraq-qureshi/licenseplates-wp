...
class URE_Utils {
   ...
   /**
    ...
    public static function filter_int_array_from_str( $input_str ) {
        ...
        $output_arr0 = explode(',', (string) $input_str );
    ...
    public static function get_int_value_from_post( $input_name, $is_array=false ) {
        
        $raw_value = ( isset( $_POST['values'][$input_name] ) ) ? URE_Base_Lib::filter_string_var( $_POST['values'][$input_name] ) : '';
        if ( $is_array ) {
            $value = empty( $raw_value ) ? array() : URE_Utils::filter_int_array_from_str( $raw_value );
        } else {
            $value = (int) $raw_value;
        }
    ...
    // end of function get_value_from_post()
    ...
    public static function mime_type($file_path) {
        if (function_exists('finfo_open')) {
            $finfo = new finfo(FILEINFO_MIME_TYPE, null);
            $mime_type = $finfo->file($file_path);
        } else if (function_exists('passthru') && function_exists('escapeshellarg')) {
            ob_start();
