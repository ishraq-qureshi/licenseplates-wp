...
?>
    <span style="font-weight: bold;"><?php echo esc_html( $tax_obj->labels->name );?></span>
<table id="ure_pva_<?php echo esc_attr( $tax_id );?>_table">
    <tr>
        <th><input type="checkbox" id="ure-cb-select-all-term-<?php echo esc_attr( $tax_id );?>" class="ure_cb_select_all_terms"></th>
        <th style="min-width: 30px;"><?php esc_html_e('ID','user-role-editor');?></th>
        <th><?php echo esc_html( $tax_obj->labels->singular_name );?></th>
    </tr>
<?php
...
<?php     
            ...
            $cb_id = 'cat_'. $category->term_id;
?>
            <input type="checkbox" name="<?php echo esc_attr( $cb_id );?>" id="<?php echo esc_attr( $cb_id );?>" class="<?php echo esc_attr( $cb_class );?>" <?php echo $checked; ?> />
        </td>
        <td><?php echo esc_html( $category->term_id );?></td>
        <td title="<?php echo esc_attr( $category->description );?>"><?php echo esc_html( $category_name );?></td>
    </tr> 
<?php
        }   // foreach($categories...
?>
...
?>        
        <tr>
            <td>
                <input type="checkbox" name="<?php echo esc_attr( $id ); ?>" id="<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $cb_class );?>" <?php echo $checked; ?>
            </td>    
            <td><?php echo esc_html( $key ); ?></td>
            <td><?php echo esc_html( $all_templates[$key] ); ?></td>
        </tr>    
<?php
        }
?>
...
<form name="ure_posts_view_access_form" id="ure_posts_view_access_form" method="POST"
    ...
    <input type="text" id="ure_posts_list" name="ure_posts_list" value="<?php echo esc_attr( $posts_list );?>" style="width: 300px;" />
    <hr/>
    
    <span style="font-weight: bold;"><?php echo esc_html_e('Authors ID list (comma separated)', 'user-role-editor');?>:</span>
    <input type="text" id="ure_posts_authors_list" name="ure_posts_authors_list" value="<?php echo esc_attr( $posts_authors_list );?>" style="width: 300px;" /><br/>
...
?>  
    ...
    <input type="hidden" name="ure_object_name" id="ure_object_name" value="<?php echo esc_attr( $ure_object_name );?>" />
<?php
        if ($ure_object_type=='role') {
?>
    <input type="hidden" name="user_role" id="ure_role" value="<?php echo esc_attr( $ure_object_name );?>" />
<?php
        }
?>
