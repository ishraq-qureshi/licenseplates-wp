...
class URE_Import_Single_Role {
    ...
    public function add_toolbar_buttons() {
        ...
        if ( is_multisite() && is_network_admin() ) {
            // Do not allow import user role from the Network Admin. Script works correctly from the single site UI only.
            return;
        }
?>
            ...
            <script type="text/javascript" >
                 jQuery.ure_postGo('<?php echo $reload_link; ?>', 
                          { action: 'roles_import_note', 
                            user_role: '<?php echo esc_js( $this->role_id ); ?>',  
                            result: <?php echo ($result->success ? 1 : 0); ?>,  
                            message: '<?php echo esc_js( $result->message ); ?>',                        
                            ure_nonce: '<?php echo $wp_nonce; ?>'} );
            </script>  
<?php
        } else {
...
<?php                                   
    ...
    static private function validate_import_to_site_request() {
        ...
        $answer = URE_Lib_Pro::check_nonce();
        if ( $answer!==TRUE ) {
            return $answer;
        }
         
        if ( !current_user_can('ure_import_roles') && !current_user_can( URE_Own_Capabilities::get_key_capability() ) ) {
            $answer = array('result'=>'error', 'message'=>esc_html__('URE: you do not have enough permissions to access this module.', 'user-role-editor') );
            return $answer;
        }        
