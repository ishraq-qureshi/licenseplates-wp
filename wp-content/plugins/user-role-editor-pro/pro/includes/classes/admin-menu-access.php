...
class URE_Admin_Menu_Access {
    ...
    private function update_menu( $key, $submenu_key, $submenu_copy, $blocked_menu_item ) {
        ...
        if ( !isset( $submenu[$submenu_key] ) || !is_array( $submenu[$submenu_key] ) ) {
            unset( $menu[$key] );
        } elseif ( count( $submenu[$submenu_key] )==0 ) {
            unset( $submenu[$submenu_key] );
            unset( $menu[$key] );   
        } elseif ( count($submenu[$submenu_key])==1 ) { // submenu has the only item
            if ( $blocked_menu_item && isset( $submenu[$submenu_key][0][2] ) &&  
                 $submenu[$submenu_key][0][2]==$submenu_key ) {  // and submenu item has the same link as the blocked menu item
    ...
    // Block "Sales Reports" menu, if WooCommerce->Reports menu item is blocked
    private function block_wc_sales_reports_menu( $blocked ) {
        global $menu;
        
        if ( in_array('831de7d47342e2e44bd540a24e526854', $blocked ) ) {    //  WooCommerce -> Reports ( admin.php?page=wc-reports )
            if ( isset( $menu['55.6'] ) ) { // Sales Reports ( admin.php?page=wc-reports )
                unset( $menu['55.6'] );
            }    
        }
                
    }
    // end of block_wc_sales_reports_menu()
    ...
    private function remove_blocked_menu_items() {
        ...
        if ( URE_Plugin_Presence::is_active('woocommerce') ) {
            $this->block_wc_sales_reports_menu( $blocked['data'] );
        }
    ...
    private function get_first_available_menu_item($dashboard_allowed) {    
        ...
        foreach ($menu as $key=>$menu_item) {
            ...
            if ( is_array( $this->blocked_urls ) && in_array( $link, $this->blocked_urls ) ) {
                continue;
            }
    ...
    public function redirect_blocked_urls() {        
        ...
        $command = $this->lib->extract_command_from_url( $request_uri );
