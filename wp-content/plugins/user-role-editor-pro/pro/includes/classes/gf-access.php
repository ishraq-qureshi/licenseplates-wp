...
class URE_GF_Access {
    ...
    public function set_final_hooks() {
        ...
        if ( URE_GF_Access_User::can_edit( $current_user ) ) {
            ...
            add_filter('gform_form_switcher_forms', array( $this, 'form_switcher_forms') );
            add_filter('get_user_metadata', array( $this, 'recent_forms_meta'), 10, 4 ); 
        }
...
<?php                    
    ...
    private function redirect_to_forms() {
        ...
        // its late to use wp_redirect() and WP sent some headers already, so use this method for redirection
?>
        <script>
...
<?php                    
    ...
    protected function _restrict_form_list($query) {
        ...
        if ( is_blog_admin() ) {  // if it's not admin dashboard - nothing to change
            $uri = trim( sanitize_url( $_SERVER['REQUEST_URI'] ) );
    ...
    // enf of restrict_form_list()
 
    /*
     * Limit forms list in the form switcher dropdown list control by the list of the forms allowed for this user
     */
    public function form_switcher_forms( $forms ) {
        
        $allowed_forms = $this->get_allowed_forms();
        if ( !empty( $allowed_forms ) ) {
            $forms = array_filter($forms, function( $form ) use ( $allowed_forms ) {
                return in_array( $form->id, $allowed_forms );
            });
        }
        
        return $forms;
    }
    // end of form_switcher_forms()

    
    /*
     * Limit recent forms to the ones that the user is allowed to see
     */
    public function recent_forms_meta( $value, $user_id, $meta_key, $single ) {
