	...
	abstract class Ai1wmve_Main_Controller {
		...
		public function ai1wmve_loaded() {
			if ( ! defined( 'AI1WM_PLUGIN_NAME' ) ) {
				...
				if ( ! has_action( Ai1wmve_Schedule_Event::CRON_HOOK, 'Ai1wmve_Schedules_Controller::run' ) ) {
				...
				// Replace the base single-file upload handler with the pro chunked handler.
				// The base handler is registered on plugins_loaded:10, this runs on
				// plugins_loaded:20 (before the extension's ai1wm_commands).
				if ( ! has_action( 'ai1wm_import', 'Ai1wmve_Import_Upload::execute' ) ) {
					remove_filter( 'ai1wm_import', 'Ai1wm_Import_Upload::execute', 5 );
					add_filter( 'ai1wm_import', 'Ai1wmve_Import_Upload::execute', 5 );
				}
