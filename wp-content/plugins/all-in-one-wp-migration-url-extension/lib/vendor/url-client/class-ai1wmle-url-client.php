...
class Ai1wmle_URL_Client {
	...
	public function get_query() {
		return (string) parse_url( $this->file_url, PHP_URL_QUERY );
	}
	...
	/**
	 * Fetch effective URL from provider via the redirect service.
	 *
	 * The service resolves provider-specific URLs (OneDrive, Google Drive,
	 * Dropbox, WeTransfer, pCloud) to a direct download URL, replacing the
	 * provider-specific rewrites that previously lived in PHP.
	 *
	 * @return string
	 * @throws Ai1wmle_Error_Exception
	 */
	public function fetch_effective_url() {
		$api = new Ai1wmle_URL_Curl;
		$api->set_base_url( AI1WMLE_URL_PROCESS_DOWNLOAD );
		$api->set_option( CURLOPT_POST, true );
		$api->set_option(
			CURLOPT_POSTFIELDS,
			json_encode(
				array(
					'ai1wmle_purchase_id' => AI1WMLE_PURCHASE_ID,
					'ai1wmle_site_url'    => get_site_url(),
					'ai1wmle_admin_email' => get_option( 'admin_email' ),
					'ai1wmle_file_url'    => $this->get_file_url(),
				)
			)
		);
