...
class Ai1wmle_URL_Curl {
	...
	public function __construct() {
		if ( ! extension_loaded( 'curl' ) ) {
			throw new Ai1wmle_Error_Exception( __( 'URL Extension requires PHP cURL extension. <a href="https://help.servmask.com/knowledgebase/curl-missing-in-php-installation/" target="_blank">Technical details</a>', AI1WMLE_PLUGIN_NAME ) );
		}
	 ...
	 * Set effective URL
	 *
	 * @param  string $value Effective URL
	 * @return object
	 */
	public function set_effective_url( $value ) {
		$this->effective_url = $value;
		return $this;
	}
	 ...
	 * Set content length
	 *
	 * @param  integer $value Content length
	 * @return object
	 */
	...
	public function make_request( $parse_as_json = false ) {
		...
		if ( $response === false ) {
			if ( ( $errno = curl_errno( $this->handler ) ) ) {
				throw new Ai1wmle_Connect_Exception( sprintf( __( 'Unable to connect to URL address. Error code: %s. <a href="https://help.servmask.com/knowledgebase/url-error-codes/#%s" target="_blank">Technical details</a>', AI1WMLE_PLUGIN_NAME ), $errno, $errno ) );
			}
		}
		...
		if ( $http_code === 429 ) {
			throw new Ai1wmle_Rate_Limit_Exception( sprintf( __( 'Too many requests. Please try again later. Error code: %s <a href="https://help.servmask.com/knowledgebase/url-error-codes/#%s" target="_blank">Technical details</a>', AI1WMLE_PLUGIN_NAME ), $http_code, $http_code ) );
		}

		if ( $http_code >= 500 ) {
			throw new Ai1wmle_Internal_Server_Error_Exception( sprintf( __( 'Internal Server Error. Please try again later. Error code: %s <a href="https://help.servmask.com/knowledgebase/url-error-codes/#%s" target="_blank">Technical details</a>', AI1WMLE_PLUGIN_NAME ), $http_code, $http_code ) );
		}

		if ( $http_code >= 400 ) {
			$data = json_decode( $response, true );
			if ( is_array( $data ) && isset( $data['message'] ) ) {
				throw new Ai1wmle_Error_Exception( $data['message'], $http_code );
			}

			if ( isset( $this->messages[ $http_code ] ) ) {
				throw new Ai1wmle_Error_Exception( sprintf( __( '%s. <a href="https://help.servmask.com/knowledgebase/url-error-codes/#%s" target="_blank">Technical details</a>', AI1WMLE_PLUGIN_NAME ), $this->messages[ $http_code ], $http_code ) );
			}

			throw new Ai1wmle_Error_Exception( sprintf( __( 'Error code: %s. <a href="https://help.servmask.com/knowledgebase/url-error-codes/#%s" target="_blank">Technical details</a>', AI1WMLE_PLUGIN_NAME ), $http_code, $http_code ) );
		}

		// Effective URL
		$this->set_effective_url( curl_getinfo( $this->handler, CURLINFO_EFFECTIVE_URL ) );

		// Content length
		$this->set_content_length( curl_getinfo( $this->handler, CURLINFO_CONTENT_LENGTH_DOWNLOAD ) );
